                ...
                <table class="table table-bordered table-sm text-center">
                    ...
                    @forelse ($kategoris as $key => $kategori)
                            <tr>
                                ...
                                <td>
                                    <a class="btn" href="{{ route('angkatan.show', $kategori->id)}}">{{ $kategori->nama_kategori }}</a>
                                </td>
                                <td>
                                    <a href="{{ route('kategori.edit', $kategori->id)}}" class="btn btn-success btn-sm">
                                        <i class='fas fa-edit'></i></a>
                                    <form action="{{ route('kategori.destroy', $kategori->id)}}" method="POST" class="pt-1">
                                            @csrf
                                            @method('DELETE')
